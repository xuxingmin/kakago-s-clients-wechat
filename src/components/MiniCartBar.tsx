  ...
  return (
    <>
      ...
      <div className="fixed bottom-16 left-4 right-4 z-40 animate-in slide-in-from-bottom duration-300">
        <div className="relative bg-gradient-to-r from-black/95 via-[#0d0d10] to-black/95 rounded-2xl border border-primary/30 shadow-[0_0_30px_hsl(var(--copper-500)/0.2)] px-4 py-3 flex items-center justify-between backdrop-blur-xl overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-primary/5 via-transparent to-accent/20 pointer-events-none" />
          <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-primary/50 to-transparent" />
          
          <button onClick={() => setIsCartOpen(true)} className="flex items-center gap-3 group relative z-10">
            <div className="relative w-11 h-11 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center group-hover:from-primary/30 group-hover:to-accent/20 transition-all duration-300 border border-primary/20">
              <ShoppingCart className="w-5 h-5 text-primary group-hover:scale-110 transition-transform" />
              <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-gradient-to-br from-primary to-accent text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-[0_0_10px_hsl(var(--copper-500)/0.35)] animate-pulse">
                {totalItems}
              </span>
            </div>
          ...
          <button
            ...
            className="relative h-11 px-6 bg-gradient-to-r from-primary via-primary to-accent rounded-xl flex items-center gap-2 text-white font-bold text-sm shadow-[0_0_25px_hsl(var(--copper-500)/0.25)] transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_0_35px_hsl(var(--copper-500)/0.2)] active:scale-[0.97] overflow-hidden z-10"
          >
      ...
      {isCartOpen && (
        <>
          <div className="fixed inset-0 bg-espresso/50 backdrop-blur-sm z-[60]" onClick={closeDrawer} />
          <div className="fixed bottom-0 left-0 right-0 z-[60] animate-in slide-in-from-bottom duration-300">
            <div className="bg-gradient-to-t from-black via-[#1a1a1d] to-[#1a1a1d]/95 rounded-t-2xl border-t border-white/10 shadow-2xl max-h-[60vh] overflow-hidden flex flex-col">
                  ...
                  return (
                    <div key={item.id} className="flex items-center justify-between bg-white/5 rounded-lg px-2.5 py-2.5">
                      <div className="flex items-center gap-2.5 flex-1 min-w-0">
                        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center flex-shrink-0 border border-primary/20 relative">
              ...
              <div className="px-4 py-3 pb-20 border-t border-white/10 bg-espresso/70">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={handleCheckout}
                    className="h-11 px-8 bg-gradient-to-r from-primary via-primary to-accent rounded-full flex items-center justify-center text-white font-bold text-base shadow-[0_0_25px_hsl(var(--copper-500)/0.25)] transition-all hover:scale-[1.03] active:scale-[0.97]"
                  >
                    {t("去结算", "Checkout")}
                  </button>
                </div>
